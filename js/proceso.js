 ...
 */
const crearProceso = (nombre, tiempo_llegada = parseInt(txtLlegada.value), rafaga = parseInt(txtRafaga.value), padre, tipo = simulador) => {
    const proceso = {
...
const handlerRR = () => {
    ...
    if (proceso.rafaga > quantum) {
        const procesoSobrante = crearProceso(`$${proceso.nombre}`, proceso.tiempo.tiempo_llegada, proceso.rafaga - proceso.tiempo.tiempo_ejecutado, proceso, proceso.tipo);
...
const handlerColaBloqueo = () => {
    if (colaBloqueados.length > 0) {
        if (colaBloqueados[0].bloqueo.tiempo_block < 5) {
            ...
            const proceso_reanudado = crearProceso(`${proceso_bloqueado.nombre}*`, proceso_bloqueado.tiempo.tiempo_llegada, proceso_bloqueado.rafaga - proceso_bloqueado.tiempo.tiempo_ejecutado, proceso_bloqueado, proceso_bloqueado.tipo);
